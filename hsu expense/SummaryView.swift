...
struct SummaryView: View {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(spacing: 8) {
                    ...
                    summaryRow(
                        ...
                        value: currencyManager.currentCurrency.format(Decimal(summaryData.weekTotalAmount)),
                        valueColor: Color.expenseGreen
                    )
                    
                    summaryRow(
                        label: "Average per Day",
                        value: currencyManager.currentCurrency.format(Decimal(summaryData.weekAveragePerDay)),
                        valueColor: Color(red: 1.0, green: 0.596, blue: 0.0) // #FF9800
                    )
                }
            }
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(spacing: 8) {
                    ...
                    summaryRow(
                        ...
                        value: currencyManager.currentCurrency.format(Decimal(summaryData.monthTotalAmount)),
                        valueColor: Color.expenseGreen
                    )
                }
            }
    ...
    private func loadSummaryData() {
        ...
        let totalAmount = expenses.reduce(0) { $0 + Double($1.price) }
        ...
        let todayTotal = todayExpenses.reduce(0) { $0 + Double($1.price) }
        ...
        let weekTotal = weekExpenses.reduce(0) { $0 + Double($1.price) }
        ...
        let monthTotal = monthExpenses.reduce(0) { $0 + Double($1.price) }
        
        // Find extremes
        let sortedExpenses = expenses.sorted { Double($0.price) > Double($1.price) }
